...
function hierarchy(g) {
  function dfs(v) {
    ...
    while (stack.length > 0) {
      ...
      if (g.node(node).rank === undefined || g.node(node).rank < rank) {
        g.node(node).rank = rank;

        _.forEachRight(g.outEdges(node), function (e) {
          stack.push([e.w, rank + 1]);
        });
      }
    }
  }
